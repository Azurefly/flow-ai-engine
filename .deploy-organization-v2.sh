...
release_archive="/tmp/flow-ai-engine-5a6e5c6.tar.gz"
test_override="/tmp/flow-ai-engine-org-test.yaml"
stamp="$(date +%Y%m%d-%H%M%S)-organization-v2"
...
sudo docker compose -f compose.yaml -f "$test_override" --profile test run --build --rm org_test
...
printf '%s\n' 5a6e5c6 | sudo tee "$deploy_root/DEPLOYED_COMMIT" >/dev/null
