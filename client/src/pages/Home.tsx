...
function FlowConsole({ user, general, onLogout }: { user: UserIdentity; general: PublicGeneral; onLogout: () => void }) {
  ...
  const runFlow = trpc.workflow.run.useMutation({
    onSuccess: result => { void utils.workflow.runs.invalidate(); void utils.workflow.runMetrics.invalidate(); setSelectedRunId(result.runId); navigateSection("runs"); toast.success(`运行完成：${result.runId.slice(0, 8)}`); },
    onError: error => toast.error(error.message),
  });
  ...
  return <main data-aiflow-console="" className="aiflow-console relative min-h-screen bg-[#f4f6f9] text-slate-700">
    ...
    <div className="flex min-h-[calc(100vh-56px)] flex-col md:flex-row">
      ...
      <section id="aiflow-console-panel" role="tabpanel" aria-labelledby={`aiflow-console-tab-${section}`} className="min-w-0 w-full flex-1">{section === "flows" && flowView === "center" && <BusinessCenter projects={(projects.data ?? []) as ProjectRecord[]} canCreate={user.role === "admin" || Boolean(access.data?.permissions?.has("workflow:create"))} onOpenProject={project => { setSelectedProject(project); setFlowView("workspace"); }} />}
        ...
        {section === "warehouse" && <WorkflowWarehouse projects={(projects.data ?? []) as ProjectRecord[]} onOpenWorkflow={(project, workflowId) => { setSelectedProject(project); setSelectedWorkflowId(workflowId); navigateSection("flows"); setFlowView("editor"); }} />}
...
function FlowDesigner({ workflow, definition, name, setName, canEdit, canPublish, canRun, canManage, members, candidates, savePending, publishPending, runPending, runInput, setRunInput, models, templates, subflows, onDefinitionChange, onBackToDesignCenter, onSave, onPublish, onRun, onExport, onImport, onDuplicate, onDelete, onSaveAsSubflow, onCreateTemplate, onUpdateTemplate, onDeleteTemplate, onToggleSubflow, onDeleteSubflow, onGrant, onRevoke }: { workflow: any; definition: Definition | null; name: string; setName: (value: string) => void; canEdit: boolean; canPublish: boolean; canRun: boolean; canManage: boolean; members: any[]; candidates: any[]; savePending: boolean; publishPending: boolean; runPending: boolean; runInput: Record<string, unknown>; setRunInput: (value: Record<string, unknown>) => void; models: Array<{ id: string; ownedBy: string }>; templates: any[]; subflows: any[]; onDefinitionChange: (definition: Definition) => void; onBackToDesignCenter: () => void; onSave: () => void; onPublish: () => void; onRun: () => void; onExport: () => void; onImport: () => void; onDuplicate: () => void; onDelete: () => void; onSaveAsSubflow: () => void; onCreateTemplate: (input: any) => void; onUpdateTemplate: (template: any, updates: any) => void; onDeleteTemplate: (id: string) => void; onToggleSubflow: (subflow: any, isEnabled: boolean) => void; onDeleteSubflow: (id: string) => void; onGrant: (userId: number, role: "owner" | "editor" | "operator" | "viewer", hours?: number) => void; onRevoke: (userId: number, role: "owner" | "editor" | "operator" | "viewer") => void }) {
