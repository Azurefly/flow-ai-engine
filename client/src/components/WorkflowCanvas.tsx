...
function ConfigFieldEditor({ field, value, fallback, disabled, runtimeOptions, onChange }: { field: FlowNodeDefinition["fields"][number]; value: unknown; fallback: unknown; disabled: boolean; runtimeOptions?: Array<{ value: string; label: string }>; onChange: (value: unknown) => void }) {
  ...
  if (runtimeOptions) { const current = String(effectiveValue ?? ""); const currentAvailable = runtimeOptions.some(option => option.value === current); return <label className="grid gap-1.5">{label}<select className={inputClass} disabled={disabled} value={current} onChange={event => onChange(event.target.value)}><option value="">请选择已启用的私有子流程</option>{current && !currentAvailable && <option value={current}>当前映射不可用 · {current}</option>}{runtimeOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select><FieldHelp help={field.help} /></label>; }
  if (field.kind === "boolean") return <label className="grid gap-1.5">{label}<span className="inline-flex h-9 items-center gap-2 rounded-md border border-slate-200 bg-white px-2.5 text-sm text-slate-700"><input type="checkbox" checked={Boolean(effectiveValue)} disabled={disabled} onChange={event => onChange(event.target.checked)} />{effectiveValue ? "是" : "否"}</span><FieldHelp help={field.help} /></label>;
...
function configFieldValue(field: FlowNodeDefinition["fields"][number], config: NodeConfig) {
...
type OriginalFieldSpec = {
  ...
  label: string;
  help?: string;
  kind?: "text" | "number" | "boolean" | "yes-no" | "structured" | "select" | "multi-select";
...
const ORIGINAL_OBJECT_FIELD_SPECS: Record<string, OriginalFieldSpec[]> = {
  ...
  sxsz: [
    { key: "zdglxgfsz", label: "自动关联相关方", help: "部门、权限部门或直属上级会作为后续接收方补入流程。", kind: "multi-select", options: [{ value: "unitWord", label: "相关方所属部门" }, { value: "authUnitWord", label: "相关方权限部门" }, { value: "upperAuthUnitWord", label: "相关方直属上级" }] },
    { key: "yrdbmsfkcz", label: "引入部门可操作", kind: "yes-no" },
    { key: "xzdzlcjywc", label: "需完成的子流程", kind: "structured" },
  ],
  fsfsz: [
    { key: "fsfbm", label: "发送方编码" },
    { key: "fsflzsf", label: "发送方流转身份", help: "决定下一节点以本人、部门还是权限部门身份继续。", kind: "select", options: [{ value: "以本人身份", label: "以本人身份" }, { value: "以部门身份", label: "以部门身份" }, { value: "以权限部门身份", label: "以权限部门身份" }] },
...
function OriginalFieldControl({ spec, value, disabled, onChange }: { spec: OriginalFieldSpec; value: unknown; disabled: boolean; onChange: (value: unknown) => void }) {
  ...
  if (spec.kind === "select") return <select className={inputClass} value={String(value ?? "")} disabled={disabled} onChange={event => onChange(event.target.value)}>{(spec.options ?? []).map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select>;
  if (spec.kind === "multi-select") {
    const selected = new Set(Array.isArray(value) ? value.map(String) : []);
    return <div className="grid gap-1 rounded border border-slate-200 bg-white p-2">{(spec.options ?? []).map(option => <label key={option.value} className="inline-flex items-center gap-2 text-xs font-normal text-slate-700"><input type="checkbox" checked={selected.has(option.value)} disabled={disabled} onChange={event => { const next = new Set(selected); event.target.checked ? next.add(option.value) : next.delete(option.value); onChange(Array.from(next)); }} />{option.label}</label>)}</div>;
  }
  return <input className={inputClass} type={spec.kind === "number" ? "number" : "text"} min={spec.min} max={spec.max} value={String(value ?? "")} disabled={disabled} onChange={event => onChange(spec.kind === "number" ? structuredValue(event.target.value) : event.target.value)} />;
}
...
function StructuredListRow({ fieldKey, item, disabled, onChange, onRemove }: { fieldKey: string; item: unknown; disabled: boolean; onChange: (value: unknown) => void; onRemove: () => void }) {
  ...
  if (fieldKey === "routes") { const route = item && typeof item === "object" ? item as NodeConfig : {}; const condition = route.condition && typeof route.condition === "object" ? route.condition as NodeConfig : {}; const roleKeys = Array.isArray(route.roleKeys) ? route.roleKeys.map(String) : []; return <div className="grid min-w-0 gap-3 rounded-lg border border-violet-100 bg-white p-3 shadow-sm"><div className="flex items-center justify-between gap-2"><span className="text-xs font-semibold text-violet-700">{String(route.label ?? "新路径")}</span><button type="button" className="grid min-h-11 min-w-11 place-items-center text-slate-400 hover:text-red-600" disabled={disabled} onClick={onRemove} aria-label="删除路由规则"><Trash2 size={14} /></button></div><div className="grid min-w-0 gap-2 sm:grid-cols-2"><label className="grid min-w-0 gap-1 text-[11px] text-slate-500">路径句柄<input className={inputClass} placeholder="例如 employee" value={String(route.handle ?? "")} disabled={disabled} onChange={event => onChange({ ...route, handle: event.target.value })} /></label><label className="grid min-w-0 gap-1 text-[11px] text-slate-500">路径名称<input className={inputClass} placeholder="例如 员工路径" value={String(route.label ?? "")} disabled={disabled} onChange={event => onChange({ ...route, label: event.target.value })} /></label></div><label className="grid min-w-0 gap-1 text-[11px] text-slate-500">目标节点<span className="flex min-h-11 items-center rounded border border-slate-200 bg-slate-50 px-2 text-xs text-slate-700">{String(route.targetNodeId ?? route.target ?? "请从此路径句柄连线到目标状态节点")}</span></label><label className="grid min-w-0 gap-1 text-[11px] text-slate-500">流程身份（逗号分隔）<input className={inputClass} placeholder="leave_employee, initiator" value={roleKeys.join(", ")} disabled={disabled} onChange={event => onChange({ ...route, roleKeys: event.target.value.split(",").map(value => value.trim()).filter(Boolean) })} /></label><details className="rounded border border-slate-100 bg-slate-50"><summary className="cursor-pointer px-2 py-3 text-[11px] font-medium text-slate-600">附加数据条件</summary><div className="grid min-w-0 gap-2 border-t border-slate-100 p-2 sm:grid-cols-[minmax(0,1fr)_110px_minmax(0,1fr)]"><input className={inputClass} placeholder="左值" value={String(condition.left ?? "")} disabled={disabled} onChange={event => onChange({ ...route, condition: { ...condition, left: event.target.value } })} /><select className={inputClass} value={String(condition.operator ?? "equals")} disabled={disabled} onChange={event => onChange({ ...route, condition: { ...condition, operator: event.target.value } })}><option value="equals">等于</option><option value="notEquals">不等于</option><option value="contains">包含</option><option value="exists">存在</option><option value="greaterThan">大于</option><option value="lessThan">小于</option></select><input className={inputClass} placeholder="右值" value={String(condition.right ?? "")} disabled={disabled} onChange={event => onChange({ ...route, condition: { ...condition, right: structuredValue(event.target.value) } })} /></div></details></div>; }
  ...
  if (["restHeaderParam", "restGetBodyParam"].includes(fieldKey)) {
    ...
    return <div className="grid grid-cols-[minmax(88px,.8fr)_minmax(0,1.2fr)_auto] gap-2"><input className={inputClass} placeholder={`${label}名称`} value={String(pair.key ?? "")} disabled={disabled} aria-label={`${label}名称`} onChange={event => onChange({ ...pair, key: event.target.value })} /><input className={inputClass} placeholder={`${label}值`} value={String(pair.value ?? "")} disabled={disabled} aria-label={`${label}值`} onChange={event => onChange({ ...pair, value: structuredValue(event.target.value) })} /><button type="button" className="text-slate-400 hover:text-red-600" disabled={disabled} onClick={onRemove} aria-label={`删除${label}`}><Trash2 size={14} /></button></div>;
  }
...
export default function WorkflowCanvas({
  ...
  const updateConfigFields = (updates: NodeConfig) => updateSelected({ config: { ...selectedConfig, ...updates } });
  const updateConfigField = (key: string, value: unknown) => updateConfigFields({ [key]: value });
    ...
    <div data-aiflow-workflow-canvas="" className={inspectorMode === "maximized" ? "grid min-h-[650px] min-w-0 max-w-full grid-cols-1 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm lg:grid-cols-[minmax(0,1fr)_620px]" : inspectorMode === "compact" ? "grid min-h-[650px] min-w-0 max-w-full grid-cols-1 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm lg:grid-cols-[minmax(0,1fr)_72px]" : "grid min-h-[650px] min-w-0 max-w-full grid-cols-1 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm lg:grid-cols-[minmax(0,1fr)_420px]"}>
      <section ref={canvasRegionRef} className="relative min-w-0 bg-slate-50">
        ...
        <div className="relative h-[420px] sm:h-[590px]" onDragOver={event => event.preventDefault()} onDrop={handleCanvasDrop}>
          <details className="absolute left-3 top-3 z-10 rounded-lg border border-slate-200 bg-white/95 text-[11px] text-slate-600 shadow-sm"><summary className="cursor-pointer px-3 py-2 font-semibold text-slate-700">画布说明</summary><div className="grid gap-2 border-t border-slate-100 px-3 py-2"><div className="flex flex-wrap gap-3"><span className="flex items-center gap-1"><i className="h-2 w-2 rounded-full bg-red-500" />未完全配置</span><span className="flex items-center gap-1"><i className="h-2 w-2 rounded-full bg-blue-500" />配置中</span><span className="flex items-center gap-1"><i className="h-2 w-2 rounded-full bg-emerald-500" />已配置</span></div><div className="flex flex-wrap gap-3 text-slate-500"><span className="flex items-center gap-1"><Move size={13} />画布移动</span><span className="flex items-center gap-1"><Maximize2 size={13} />画布缩放</span><span className="flex items-center gap-1"><MousePointer2 size={13} />选择连线</span><span className="flex items-center gap-1"><Square size={12} />节点框选</span></div></div></details>
      ...
      <aside data-workflow-inspector className="border-t border-slate-200 bg-white lg:border-l lg:border-t-0">
        ...
        {inspectorMode !== "compact" && (selected && selectedDefinition ? <div className="max-h-[650px] overflow-y-auto">
          <div className="space-y-3 p-4">
            <label className="grid gap-1.5 text-xs font-medium text-slate-600">节点名称<input className="h-9 rounded-lg border border-slate-200 px-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 disabled:bg-slate-50" value={String(selected.data.label ?? "")} disabled={inspectorDisabled} onChange={event => updateSelected({ label: event.target.value })} /></label>
            <div className={`rounded-lg border px-3 py-2 ${selectedConfigState === "partial" ? "border-red-200 bg-red-50" : selectedConfigState === "editing" ? "border-blue-200 bg-blue-50" : "border-emerald-200 bg-emerald-50"}`}><p className="text-xs font-semibold text-slate-700">{selectedDefinition.label} · {selectedConfigState === "partial" ? "未完全配置" : selectedConfigState === "editing" ? "配置中" : "已配置"}</p></div>
          </div>
          <div role="tablist" aria-label={`${selectedDefinition.label}配置分类`} className="flex gap-1 overflow-x-auto border-y border-slate-100 bg-slate-50 px-3 py-2">
            {selectedFieldGroups.map(group => <button key={group.label} type="button" role="tab" aria-selected={activeInspectorGroup?.label === group.label} className={`shrink-0 rounded-lg px-3 py-1.5 text-xs font-medium ${activeInspectorGroup?.label === group.label ? "bg-[#3370ed] text-white shadow-sm" : "bg-white text-slate-600 hover:bg-slate-100"}`} onClick={() => setInspectorTab(group.label)}>{group.label}</button>)}
          </div>
          {activeInspectorGroup && <section role="tabpanel" className="m-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div className="mb-4"><h3 className="text-sm font-semibold text-slate-800">{activeInspectorGroup.label}</h3><p className="mt-1 text-[11px] leading-5 text-slate-500">{activeInspectorGroup.description}</p></div><div className="space-y-4">{activeInspectorGroup.fields.map(field => { const fieldValue = configFieldValue(field, selectedConfig); const runtimeOptions = field.key === "subflowId" ? subflows.filter(subflow => subflow.isEnabled).map(subflow => ({ value: subflow.id, label: subflow.name })) : undefined; return <ConfigFieldEditor key={`${selected.id}-${field.key}-${JSON.stringify(fieldValue ?? selectedDefaults[field.key])}`} field={field} value={fieldValue} fallback={selectedDefaults[field.key]} disabled={inspectorDisabled} runtimeOptions={runtimeOptions} onChange={value => { if (field.key !== "subflowId") return updateConfigField(field.key, value); const selectedSubflow = subflows.find(subflow => subflow.id === value && subflow.isEnabled); updateConfigFields({ subflowId: value, zlcxz: selectedSubflow ? { id: selectedSubflow.id, text: selectedSubflow.name } : { id: "", text: "" } }); }} />; })}</div></section>}
          {!readOnly && <div className="p-4 pt-0"><Button type="button" size="sm" variant="outline" className="w-full rounded-lg" disabled={inspectorDisabled} onClick={() => { const name = window.prompt("节点模板名称", String(selected.data.label ?? "未命名模板")); if (name?.trim()) onSaveTemplate?.({ name: name.trim(), nodeType: selected.data.kind as ReuseTemplate["nodeType"], config: selectedConfig }); }}><Save size={13} />保存为节点模板</Button></div>}
        </div> : selectedEdgeId ? <div className="m-4 mt-8 rounded-lg border border-indigo-200 bg-indigo-50 p-4 text-sm leading-6 text-indigo-800"><p className="font-semibold">已选中连线</p><p className="mt-1 text-xs">可点击顶部“删除连线”，或按 Delete / Backspace。删除后可立即撤销。</p></div> : <div className="m-4 mt-8 rounded-lg border border-dashed border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-500"><p className="font-medium text-slate-700">暂无配置信息</p><p className="mt-1">请选择画布中的元件查看配置信息；若无元件，请添加元件。</p></div>)}
