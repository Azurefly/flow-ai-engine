...
function ConfigFieldEditor({ field, value, fallback, disabled, onChange }: { field: FlowNodeDefinition["fields"][number]; value: unknown; fallback: unknown; disabled: boolean; onChange: (value: unknown) => void }) {
  ...
  if (field.kind === "json") return <JsonField key={JSON.stringify(effectiveValue)} field={field} value={effectiveValue} disabled={disabled} onChange={onChange} />;
  if (field.kind === "template" && effectiveValue && typeof effectiveValue === "object") return <JsonField key={JSON.stringify(effectiveValue)} field={field} value={effectiveValue} disabled={disabled} onChange={onChange} />;
...
function JsonField({ field, value, disabled, onChange }: { field: FlowNodeDefinition["fields"][number]; value: unknown; disabled: boolean; onChange: (value: unknown) => void }) {
  const [draft, setDraft] = useState(() => JSON.stringify(value ?? {}, null, 2));
  useEffect(() => setDraft(JSON.stringify(value ?? {}, null, 2)), [value]);
  return <label className="grid gap-1.5"><span className="flex items-center gap-1 text-xs font-semibold text-slate-700">{field.required && <i className="not-italic text-red-500">*</i>}{field.label}</span><textarea className="min-h-28 w-full rounded-md border border-slate-200 bg-slate-950 p-2.5 font-mono text-[11px] leading-5 text-emerald-200 outline-none transition focus:border-indigo-400 disabled:opacity-70" value={draft} disabled={disabled} onChange={event => setDraft(event.target.value)} onBlur={() => { try { onChange(JSON.parse(draft)); } catch { window.alert(`${field.label}必须是合法的 JSON。`); } }} /><FieldHelp help={field.help} /></label>;
}
