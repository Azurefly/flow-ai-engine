...
export default function WorkflowGovernance({ workflowId, canEdit, canPublish }: { workflowId: string; canEdit: boolean; canPublish: boolean }) {
  ...
  return <><section data-aiflow-process-detail="" className="mt-4 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
    <div className="border-b border-slate-100 p-4"><p className="text-[10px] font-bold tracking-[.18em] text-blue-600">PROCESS DETAIL</p><div className="mt-1 flex flex-wrap items-end justify-between gap-2"><div><h3 className="text-sm font-semibold text-slate-900">流程引导与基本信息</h3><p className="mt-1 text-xs text-slate-500">原始详情页的设计、审核、发布和实例运行路径；每项状态均来自当前流程真实数据。</p></div><span className="rounded border border-slate-200 bg-slate-50 px-2 py-1 text-[11px] text-slate-600">{item?.flowType === "data" ? "数据流程" : item?.flowType === "control" ? "控制流程" : "状态流程"}</span></div><AuditResetButton projectId={item?.projectId} workflowId={workflowId} auditStatus={item?.auditStatus} status={item?.status} /></div>
