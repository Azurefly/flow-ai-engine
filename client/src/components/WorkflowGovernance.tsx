...
export default function WorkflowGovernance({ workflowId, canEdit, canPublish }: { workflowId: string; canEdit: boolean; canPublish: boolean }) {
  ...
  return <><section data-aiflow-process-detail="" className="mt-4 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
    ...
    <div className="border-t border-dotted border-[#dfe4ed]"><div className="px-4 pt-3 text-sm font-semibold text-slate-900">基本信息</div><div className="grid gap-3 p-4 sm:grid-cols-3"><div><p className="text-[10px] font-bold tracking-[.12em] text-slate-400">流程名称</p><p className="mt-1 truncate text-sm font-medium text-slate-700">{item?.name ?? "正在读取…"}</p></div><div><p className="text-[10px] font-bold tracking-[.12em] text-slate-400">流程说明</p><p className="mt-1 truncate text-sm text-slate-600">{item?.description || "未填写流程说明"}</p></div><div><p className="text-[10px] font-bold tracking-[.12em] text-slate-400">详细配置</p><Button type="button" variant="outline" size="sm" className="mt-1 h-7 text-xs" onClick={() => window.dispatchEvent(new Event("flow:inspect-node"))}>查看详细配置</Button></div></div></div>
