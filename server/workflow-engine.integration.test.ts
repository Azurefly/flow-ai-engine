...
const username = `workflow_engine_${randomUUID().slice(0, 8)}`;
...
const definition: Definition = {
  ...
  edges: [
    ...
    { id: "condition-end-false", sourceNodeId: "condition", sourceHandle: "false", targetNodeId: "end" },
  ],
};
...
describe("工作流引擎真实 HTTP 集成", () => {
  ...
  runIntegration("可在服务器端完成 HTTP、转换、条件与结束节点并写入节点日志", async () => {
    ...
    await pool.query(
      ...
      [`test:${username}`, username, "工作流引擎测试用户", "admin", "active", "internal"]
    );
    ...
    await pool.query("INSERT INTO workflow (id,ownerUserId,name,description,flowType,status,definitionVersion,definitionJson) VALUES (?,?,?,'integration test','control','published',1,?)", [workflowId, user.id, "工作流引擎 HTTP 集成测试", JSON.stringify(definition)]);
