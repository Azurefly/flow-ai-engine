...
describe("原始节点配置统一契约", () => {
  ...
  it("为开始、结束、状态、操作、路由、REST、方法、表单、SQL、条件、LLM 和子流程提供可追溯默认字段", () => {
    expect(createDefaultNodeConfig("start")).toMatchObject({
      initialVariables: {},
    });
    expect(createDefaultNodeConfig("end")).toMatchObject({
      resultTemplate: "{{vars}}",
    });
    expect(createDefaultNodeConfig("state")).toMatchObject({
    ...
    expect(createDefaultNodeConfig("router")).toMatchObject({
      routes: [],
      defaultRoute: "default",
    });
    expect(createDefaultNodeConfig("rest")).toMatchObject({
      nodeDh: "",
      restmc: "",
      restApi: "",
      restType: "POST",
    ...
    expect(createDefaultNodeConfig("method")).toMatchObject({
      nodeDh: "",
      restmc: "",
      restApi: "",
      restType: "POST",
    ...
    expect(createDefaultNodeConfig("condition")).toMatchObject({
      left: "{{input.value}}",
      operator: "equals",
      trueHandle: "true",
      falseHandle: "false",
    });
    expect(createDefaultNodeConfig("llm")).toMatchObject({
      systemPrompt: expect.any(String),
      prompt: "{{input.prompt}}",
      maxTokens: 1024,
    });
    expect(createDefaultNodeConfig("subflow")).toMatchObject({
      subflowId: "",
      input: "{{input}}",
    });
  });
  ...
  it("允许保存未完整配置的设计草稿，但在创建可执行定义时精确阻断缺失字段", () => {
    ...
    draft.nodes.splice(1, 0, {
      ...
      config: createDefaultNodeConfig("rest"),
    });
    draft.edges = [
      {
        id: "start-rest",
    ...
    expect(() =>
      validate(draft, { flowType: "state", executable: false })
    ).not.toThrow();
    expect(() =>
      validate(draft, { flowType: "state", executable: true })
    ).toThrow("REST 节点必须配置请求地址");
  });

  it("保留历史或安装包导入定义的未知字段，同时填充缺失的已知默认值", () => {
    const merged = withNodeConfigDefaults("state", {
      stateCode: "APPROVED",
      originalExtension: { color: "#3370ED" },
    });
  ...
  it("严格验证路由分支、表单结构、REST 方法和条件句柄", () => {
    ...
    ).toThrow("路由规则必须配置分支句柄");
    expect(() =>
      validateNodeConfig("form", {
        fields: [
    ...
    expect(() =>
      validateNodeConfig("rest", {
        ...
        restType: "GET",
        restHeaderParam: [{ key: "x-token", value: "demo" }],
      })
    ).not.toThrow();
    expect(() =>
      validateNodeConfig("method", {
  ...
  it("严格限制 LLM 的 Token、超时、Schema 和失败分支配置", () => {
    ...
    expect(() =>
      validateNodeConfig("llm", { ...valid, maxTokens: 63 })
    ).toThrow("64 至 8,192");
    expect(() =>
      validateNodeConfig("llm", { ...valid, maxTokens: 8_193 })
    ).toThrow("64 至 8,192");
    expect(() =>
      validateNodeConfig("llm", { ...valid, timeoutMs: 999 })
    ).toThrow("1,000 至 120,000");
    expect(() =>
      validateNodeConfig("llm", { ...valid, timeoutMs: 120_001 })
    ).toThrow("1,000 至 120,000");
    expect(() =>
      validateNodeConfig("llm", { ...valid, outputSchema: [] })
  ...
  it("接纳结构化原版权限配置，但继续阻止任意代码和未迁移路由被静默执行", () => {
    ...
    expect(() =>
      validateNodeConfig(
        ...
        withNodeConfigDefaults("operate", {
          ...
          zdzx: {
            sfzdzx: "是",
            tjsz: [{ left: "{{input.days}}", operator: "lessThan", right: 3 }],
            code: [],
          },
        })
      )
    ...
    expect(() =>
      validateNodeConfig(
        "router",
        withNodeConfigDefaults("router", {
          nodeDh: "ROUTE1",
          lymc: "旧路由",
          lysz: [
            {
              routerTargetyId: "approved",
              route: { routerJavaCode: "return true;" },
            },
          ],
          routes: [],
        })
          ...
          lysz: [
            {
              ...
              route: {
                ...
                routerTargetId: "approved",
                routerAuthGroups: [
                  { authReceiver: { receiverRole: ["manager"] } },
                ],
              },
            },
          ],
  ...
  it("兼容安装包历史定义中的对象结束模板、简写表单字段和 code/target 路由", () => {
    expect(() =>
      validateNodeConfig("end", {
        resultTemplate: { result: "{{vars.value}}" },
      })
    ).not.toThrow();
    expect(() =>
      validateNodeConfig("form", {
    ...
    expect(() =>
      validateNodeConfig("router", {
        defaultRoute: "default",
        routes: [{ code: "default", target: "end" }],
      })
    ).not.toThrow();
    expect(
      withNodeConfigDefaults("end", { resultTemplate: undefined })
        .resultTemplate
    ).toBe("{{vars}}");
  });

  it("填充原版 REST 默认值时保留未知安装包字段", () => {
    const merged = withNodeConfigDefaults("rest", {
      restApi: "https://example.com/api",
      originalExtension: { responseMode: "legacy" },
    });
    expect(merged).toMatchObject({
      restApi: "https://example.com/api",
      restType: "POST",
  ...
  it("将历史 SQL 字段规范化为 statement 并保持兼容校验", () => {
    expect(
      withNodeConfigDefaults("sql", {
