...
describe("P1 人工任务与服务端续跑", () => {
  afterAll(async () => {
    ...
    await pool.query("DELETE FROM authorization_audit_log WHERE actorUserId IN (?, ?, ?, ?) OR targetUserId IN (?, ?, ?, ?)", [owner?.id ?? -1, operator?.id ?? -1, handoverUser?.id ?? -1, outsider?.id ?? -1, owner?.id ?? -1, operator?.id ?? -1, handoverUser?.id ?? -1, outsider?.id ?? -1]);
    await pool.query("DELETE FROM users WHERE username IN (?,?,?,?)", [`p1_owner_${suffix}`, `p1_operator_${suffix}`, `p1_handover_${suffix}`, `p1_outsider_${suffix}`]);
    await pool.end();
  });

  runIntegration("指定项目运行者可移交、退回、批量领取并完成 operate 任务，外部用户看不到任务，完成后由服务端续跑", async () => {
    pool = mysql.createPool(process.env.DATABASE_URL!);
    await pool.query("INSERT INTO users (openId,username,name,role,status,loginMethod,lastSignedIn) VALUES ?", [[[`test:p1-owner-${suffix}`, `p1_owner_${suffix}`, "P1 所有者", "admin", "active", "internal", new Date()], [`test:p1-operator-${suffix}`, `p1_operator_${suffix}`, "P1 处理人", "user", "active", "internal", new Date()], [`test:p1-handover-${suffix}`, `p1_handover_${suffix}`, "P1 移交处理人", "user", "active", "internal", new Date()], [`test:p1-outsider-${suffix}`, `p1_outsider_${suffix}`, "P1 外部用户", "user", "active", "internal", new Date()]]]);
    const [users] = await pool.query<mysql.RowDataPacket[]>("SELECT * FROM users WHERE username IN (?,?,?,?)", [`p1_owner_${suffix}`, `p1_operator_${suffix}`, `p1_handover_${suffix}`, `p1_outsider_${suffix}`]);
    owner = users.find(user => user.username === `p1_owner_${suffix}`);
    operator = users.find(user => user.username === `p1_operator_${suffix}`);
    handoverUser = users.find(user => user.username === `p1_handover_${suffix}`);
    ...
    await grantProjectMember(owner, { projectId, userId: handoverUser.id, role: "operator" });
    ...
    const assignees: any[] = await callerFor(operator).task.assignees({ taskId: waiting.taskId });
    expect(assignees.map(item => item.id)).toEqual(expect.arrayContaining([operator.id, handoverUser.id]));
    ...
    const handedOver: any = await callerFor(operator).task.handover({ taskId: waiting.taskId, targetUserId: handoverUser.id });
    expect(handedOver).toMatchObject({ status: "pending", assignedUserId: handoverUser.id, claimedByUserId: null });
    expect(await callerFor(operator).task.list({ view: "todo" })).toHaveLength(0);
    expect(await callerFor(handoverUser).task.list({ view: "todo" })).toHaveLength(1);
    const handoverClaimed: any = await callerFor(handoverUser).task.claim({ taskId: waiting.taskId });
    expect(handoverClaimed.status).toBe("claimed");
    const returned: any = await callerFor(handoverUser).task.returnToPending({ taskId: waiting.taskId });
    expect(returned).toMatchObject({ status: "pending", assignedUserId: handoverUser.id, claimedByUserId: null });
    const batchClaimed: any[] = await callerFor(handoverUser).task.batchClaim({ taskIds: [waiting.taskId] });
    expect(batchClaimed).toEqual([{ taskId: waiting.taskId, success: true }]);
    const batchCompleted: any[] = await callerFor(handoverUser).task.batchComplete({ taskIds: [waiting.taskId], result: { decision: "approved", comment: "资料完整" } });
    expect(batchCompleted).toMatchObject([{ taskId: waiting.taskId, success: true, runId: waiting.runId, status: "success" }]);
    const completed: any = await callerFor(owner).workflow.runDetail({ runId: waiting.runId });
    const done = await callerFor(handoverUser).task.list({ view: "done" });
    expect(done[0]).toMatchObject({ id: waiting.taskId, status: "completed", completedByUserId: handoverUser.id });
    expect(completed.status).toBe("success");
    expect(completed.nodeRuns.map((node: any) => node.nodeId).sort()).toEqual(["end", "operate", "start"]);
    expect(completed.nodeRuns.every((node: any) => node.status === "success")).toBe(true);
    expect(Number(completed.durationMs)).toBeGreaterThanOrEqual(Number(completed.nodeRuns.find((node: any) => node.nodeId === "operate")?.durationMs ?? 0));
    const [audits] = await pool.query<mysql.RowDataPacket[]>("SELECT detailsJson FROM authorization_audit_log WHERE resourceType='workflow_task' AND resourceId=? ORDER BY createdAt", [waiting.taskId]);
    expect(audits.map(row => (typeof row.detailsJson === "string" ? JSON.parse(row.detailsJson) : row.detailsJson).operation)).toEqual(expect.arrayContaining(["task_handover", "task_returned_to_pending", "task_claimed", "task_completed"]));
  }, 120_000);
});
