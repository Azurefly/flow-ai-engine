...
export function isTaskActor(userId: number, task: mysql.RowDataPacket) {
  return (
    [task.assignedUserId, task.claimedByUserId, task.completedByUserId].some(
...
export async function handoverWorkflowTask(
  ...
  try {
    ...
    const [result] = await connection.query<mysql.ResultSetHeader>(
      "UPDATE workflow_task SET assignedUserId=?,candidateUserIdsJson=?,status='pending',claimedByUserId=NULL,claimedAt=NULL,ownerVersion=ownerVersion+1 WHERE id=? AND status=? AND ownerVersion=?",
      [
        input.targetUserId,
