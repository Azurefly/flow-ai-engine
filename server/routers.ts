...
import { assignOrganizationMember, createOrganizationUnit, listOrganization, removeOrganizationMember, updateOrganizationUnit } from "./organization-service";
...
export const appRouter = router({
  ...
  iam: router({
    ...
    previewUserCreation: adminProcedure.input(z.object({ username: z.string().trim().max(64).optional(), name: z.string().trim().min(1).max(160), email: z.string().email().optional(), role: z.enum(["user", "admin"]).default("user"), organizationHint: z.string().trim().max(160).optional(), managerHint: z.string().trim().max(160).optional() })).mutation(({ input }) => previewUserCreation(input)),
  ...
  task: router({
    ...
    execute: protectedProcedure.input(z.object({ taskId: z.string().uuid(), result: z.record(z.string(), z.unknown()).default({ decision: "approved" }) })).mutation(({ ctx, input }) => executeWorkflowTask(ctx.user, input.taskId, input.result)),
  ...
  config: router({
    ...
    createOrganizationUnit: adminProcedure.input(z.object({ code: z.string().trim().min(2).max(64), name: z.string().trim().min(1).max(160), parentUnitId: z.string().uuid().nullable().optional(), managerUserId: z.number().int().positive().nullable().optional() })).mutation(async ({ ctx, input }) => ({ id: await createOrganizationUnit(ctx.user, input) })),
    updateOrganizationUnit: adminProcedure.input(z.object({ id: z.string().uuid(), name: z.string().trim().min(1).max(160).optional(), parentUnitId: z.string().uuid().nullable().optional(), managerUserId: z.number().int().positive().nullable().optional(), status: z.enum(["active", "disabled"]).optional() })).mutation(async ({ ctx, input }) => ({ success: await updateOrganizationUnit(ctx.user, input) })),
    assignOrganizationMember: adminProcedure.input(z.object({ unitId: z.string().uuid(), userId: z.number().int().positive(), title: z.string().trim().max(160).optional(), isPrimary: z.boolean().optional() })).mutation(async ({ ctx, input }) => ({ success: await assignOrganizationMember(ctx.user, input) })),
    removeOrganizationMember: adminProcedure.input(z.object({ unitId: z.string().uuid(), userId: z.number().int().positive() })).mutation(async ({ ctx, input }) => ({ success: await removeOrganizationMember(ctx.user, input) })),
  }),
  workflow: router({
