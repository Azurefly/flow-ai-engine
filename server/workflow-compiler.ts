...
export type WorkflowExecutionPlan = {
  ...
  outgoing: Record<string, Array<{ edgeId: string; handle: string; targetNodeId: string }>>;
  incoming: Record<string, Array<{ edgeId: string; sourceNodeId: string; handle: string }>>;
  topologicalOrder: string[] | null;
  parallelGroups: Array<{ forkNodeId: string; joinNodeId: string; branchEdgeIds: string[] }>;
  loops: Array<{ edgeId: string; sourceNodeId: string; targetNodeId: string; maxIterations: number }>;
};

export type WorkflowCompileResult =
  | { ok: true; diagnostics: []; definition: WorkflowDefinition; plan: WorkflowExecutionPlan; planHash: string }
...
export function hashWorkflowExecutionPlan(plan: WorkflowExecutionPlan) {
  return createHash("sha256").update(stableWorkflowJson(plan), "utf8").digest("hex");
}
  ...
  return {
    ...
    nodes: value.nodes
      .map(node => ({
        ...
        config: JSON.parse(JSON.stringify(withNodeConfigDefaults(node.type, node.config))),
      }))
    ...
    edges: value.edges
      .map(edge => ({
        ...
        ...(edge.loop ? { loop: { maxIterations: Number(edge.loop.maxIterations) } } : {}),
      }))
      .sort((left, right) => left.id.localeCompare(right.id)),
  };
}

function stronglyConnectedComponents(nodeIds: string[], outgoing: Map<string, WorkflowEdge[]>) {
  let index = 0;
  const stack: string[] = [];
  ...
  const visit = (nodeId: string) => {
    ...
    for (const edge of outgoing.get(nodeId) ?? []) {
      if (!indices.has(edge.targetNodeId)) {
        ...
        lowLinks.set(nodeId, Math.min(lowLinks.get(nodeId)!, lowLinks.get(edge.targetNodeId)!));
      } else if (onStack.has(edge.targetNodeId)) {
        lowLinks.set(nodeId, Math.min(lowLinks.get(nodeId)!, indices.get(edge.targetNodeId)!));
      }
    }
...
function canReachTarget(
  ...
  while (queue.length) {
    ...
    visited.add(nodeId);
    for (const edge of outgoing.get(nodeId) ?? []) queue.push(edge.targetNodeId);
  }
  return false;
}
...
export function analyzeWorkflowDefinition(
  ...
  if (!value || !Array.isArray(value.nodes) || !Array.isArray(value.edges)) {
    return { ok: false, diagnostics: [diagnostic("WF_DEF_INVALID", "流程定义格式无效。") ] };
  }
  if (
    !value.viewport ||
    ...
    diagnostics.push(diagnostic("WF_VIEWPORT_INVALID", "流程视口格式无效。", { kind: "definition", field: "viewport" }));
  }
  ...
  for (const rawNode of value.nodes) {
    ...
    const location: WorkflowDiagnosticLocation = { kind: "node", ...(nodeId ? { nodeId } : {}) };
    if (
      !node ||
      ...
      diagnostics.push(diagnostic("WF_NODE_INVALID", "流程节点格式或类型无效。", location));
      continue;
    }
    if (seenNodeIds.has(node.id))
      diagnostics.push(diagnostic("WF_NODE_ID_DUPLICATE", `节点 ID 不可重复：${node.id}。`, location));
    seenNodeIds.add(node.id);
    if (!node.position || !Number.isFinite(node.position.x) || !Number.isFinite(node.position.y))
      diagnostics.push(diagnostic("WF_NODE_POSITION_INVALID", "流程节点位置无效。", { ...location, field: "position" }));
    if (!node.config || typeof node.config !== "object" || Array.isArray(node.config)) {
      diagnostics.push(diagnostic("WF_NODE_CONFIG_INVALID", "流程节点配置必须是 JSON 对象。", { ...location, field: "config" }));
      continue;
    }
    if (executable) {
      try {
  ...
  if (starts.length !== 1 || ends.length !== 1)
    diagnostics.push(diagnostic("WF_START_END_CARDINALITY", "流程必须且仅能包含一个开始节点和一个结束节点。"));
  ...
  const outgoing = new Map(validNodes.map(node => [node.id, [] as WorkflowEdge[]]));
  const incoming = new Map(validNodes.map(node => [node.id, [] as WorkflowEdge[]]));
  ...
  for (const rawEdge of value.edges) {
    ...
    const location: WorkflowDiagnosticLocation = { kind: "edge", ...(edgeId ? { edgeId } : {}) };
    if (
      !edge ||
      ...
      diagnostics.push(diagnostic("WF_EDGE_DANGLING", "流程连线引用了不存在的节点。", location));
      continue;
    }
    if (edgeIds.has(edge.id))
      diagnostics.push(diagnostic("WF_EDGE_ID_DUPLICATE", `流程连线 ID 不可重复：${edge.id}。`, location));
    edgeIds.add(edge.id);
    if (edge.sourceNodeId === edge.targetNodeId)
      diagnostics.push(diagnostic("WF_EDGE_SELF_LOOP", `流程不允许节点自环：${edge.sourceNodeId}。`, location));
    ...
    if (edgeKeys.has(edgeKey))
      diagnostics.push(diagnostic("WF_EDGE_DUPLICATE", `流程不允许重复连线：${edge.sourceNodeId} → ${edge.targetNodeId}。`, location));
    ...
    if (edge.loop) {
      ...
      if (!Number.isInteger(maxIterations) || maxIterations < 1 || maxIterations > 1000)
        diagnostics.push(diagnostic("WF_LOOP_LIMIT_INVALID", "循环边 maxIterations 必须是 1 到 1000 的整数。", { ...location, field: "loop.maxIterations" }));
    }
  ...
  if (executable && starts.length === 1 && ends.length === 1) {
    ...
    if ((incoming.get(startId) ?? []).length)
      diagnostics.push(diagnostic("WF_START_HAS_INCOMING", "开始节点不允许存在入边。", { kind: "node", nodeId: startId }));
    if ((outgoing.get(endId) ?? []).length)
      diagnostics.push(diagnostic("WF_END_HAS_OUTGOING", "结束节点不允许存在出边。", { kind: "node", nodeId: endId }));
    if (!(outgoing.get(startId) ?? []).length)
      diagnostics.push(diagnostic("WF_START_NO_OUTGOING", "开始节点必须连接后继节点。", { kind: "node", nodeId: startId }));
    ...
    value.nodes
      ...
      .forEach(node => diagnostics.push(diagnostic("WF_NODE_UNREACHABLE", `存在从开始节点不可达的节点：${node.name || node.id}。`, { kind: "node", nodeId: node.id })));
    ...
    value.nodes
      ...
      .forEach(node => diagnostics.push(diagnostic("WF_NODE_CANNOT_REACH_END", `存在无法到达结束节点的路径：${node.name || node.id}。`, { kind: "node", nodeId: node.id })));

    for (const node of validNodes) {
      const nodeOutgoing = outgoing.get(node.id) ?? [];
      if (node.type === "condition") {
        for (const handle of ["true", "false"]) {
          const matching = nodeOutgoing.filter(edge => (edge.sourceHandle?.trim() || "default") === handle);
          if (matching.length !== 1)
            diagnostics.push(diagnostic("WF_CONDITION_BRANCH_INVALID", `条件节点“${node.name}”必须且仅能连接一个 ${handle} 分支。`, { kind: "node", nodeId: node.id, field: handle }));
        }
        nodeOutgoing
      ...
      if (node.type === "router") {
        ...
        const routeHandles = routes
          .map(route => (route && typeof route === "object" ? String((route as Record<string, unknown>).handle ?? (route as Record<string, unknown>).code ?? "").trim() : ""))
          .filter(Boolean);
        const defaultHandle = String(node.config.defaultRoute ?? "default").trim() || "default";
        const configuredHandles = new Set([...routeHandles, defaultHandle]);
        if (new Set(routeHandles).size !== routeHandles.length)
          diagnostics.push(diagnostic("WF_ROUTER_HANDLE_DUPLICATE", `路由节点“${node.name}”的分支句柄不可重复。`, { kind: "node", nodeId: node.id, field: "config.routes" }));
        configuredHandles.forEach(handle => {
          if (!nodeOutgoing.some(edge => (edge.sourceHandle?.trim() || "default") === handle))
            diagnostics.push(diagnostic("WF_ROUTER_BRANCH_UNCONNECTED", `路由节点“${node.name}”的分支 ${handle} 未连接目标节点。`, { kind: "node", nodeId: node.id, field: `handle:${handle}` }));
        });
        nodeOutgoing
          .filter(edge => !configuredHandles.has(edge.sourceHandle?.trim() || "default"))
          .forEach(edge => diagnostics.push(diagnostic("WF_ROUTER_HANDLE_UNKNOWN", `路由节点存在未配置的分支句柄：${edge.sourceHandle || "default"}。`, { kind: "edge", edgeId: edge.id, field: "sourceHandle" })));
        ...
        if (broadcast) {
          ...
          if (!joinNodeId || !nodesById.has(joinNodeId)) {
            diagnostics.push(diagnostic("WF_PARALLEL_JOIN_REQUIRED", `并行路由“${node.name}”必须配置有效的 parallelJoinNodeId。`, { kind: "node", nodeId: node.id, field: "config.parallelJoinNodeId" }));
          } else {
            const joinNode = nodesById.get(joinNodeId)!;
            if (String(joinNode.config.parallelForNodeId ?? "").trim() !== node.id)
              diagnostics.push(diagnostic("WF_PARALLEL_JOIN_MISMATCH", `汇聚节点“${joinNode.name}”必须用 parallelForNodeId 反向绑定并行路由 ${node.id}。`, { kind: "node", nodeId: joinNodeId, field: "config.parallelForNodeId" }));
            if ((incoming.get(joinNodeId) ?? []).length < 2)
              diagnostics.push(diagnostic("WF_PARALLEL_JOIN_INPUTS_REQUIRED", `汇聚节点“${joinNode.name}”至少需要两条入边。`, { kind: "node", nodeId: joinNodeId }));
            for (const edge of nodeOutgoing) {
              if (!canReachTarget(edge.targetNodeId, joinNodeId, outgoing, node.id))
                diagnostics.push(diagnostic("WF_PARALLEL_BRANCH_MISSES_JOIN", `并行分支 ${edge.id} 无法到达汇聚节点“${joinNode.name}”。`, { kind: "edge", edgeId: edge.id }));
            }
          }
        }
      }
    ...
    const components = stronglyConnectedComponents(Array.from(nodeIds), outgoing);
    for (const component of components.filter(item => item.length > 1)) {
      const memberIds = new Set(component);
      const componentEdges = validEdges.filter(edge => memberIds.has(edge.sourceNodeId) && memberIds.has(edge.targetNodeId));
      const loopEdges = componentEdges.filter(edge => edge.loop);
      if (!loopEdges.length) {
        const node = nodesById.get(component[0]!);
        diagnostics.push(diagnostic("WF_LOOP_NOT_DECLARED", `流程存在未声明执行语义的循环：${node?.name || component[0]}。`, { kind: "node", nodeId: component[0] }));
      }
    }
    validEdges
  ...
  const normalizedOutgoing = new Map(normalized.nodes.map(node => [node.id, [] as WorkflowEdge[]]));
  const normalizedIncoming = new Map(normalized.nodes.map(node => [node.id, [] as WorkflowEdge[]]));
  ...
  const loopEdgeIds = new Set(normalized.edges.filter(edge => edge.loop).map(edge => edge.id));
  const indegrees = new Map(normalized.nodes.map(node => [node.id, 0]));
  normalized.edges.filter(edge => !loopEdgeIds.has(edge.id)).forEach(edge => indegrees.set(edge.targetNodeId, (indegrees.get(edge.targetNodeId) ?? 0) + 1));
  const topologyQueue = normalized.nodes.map(node => node.id).filter(nodeId => indegrees.get(nodeId) === 0).sort();
  ...
  while (topologyQueue.length) {
    ...
    for (const edge of normalizedOutgoing.get(nodeId) ?? []) {
      ...
      if (indegrees.get(edge.targetNodeId) === 0) topologyQueue.push(edge.targetNodeId);
    }
    topologyQueue.sort();
  }
  const parallelGroups = normalized.nodes
    .filter(node => node.type === "router" && (node.config.gbms === true || node.config.gbms === "true" || node.config.broadcast === true || node.config.broadcast === "true"))
    .map(node => ({
      forkNodeId: node.id,
      joinNodeId: String(node.config.parallelJoinNodeId),
      branchEdgeIds: (normalizedOutgoing.get(node.id) ?? []).map(edge => edge.id).sort(),
    }));
  const plan: WorkflowExecutionPlan = {
    ...
    terminalNodeIds: normalized.nodes.filter(node => node.type === "end").map(node => node.id).sort(),
    outgoing: Object.fromEntries(
      normalized.nodes.map(node => [
        node.id,
        (normalizedOutgoing.get(node.id) ?? []).map(edge => ({ edgeId: edge.id, handle: edge.sourceHandle || "default", targetNodeId: edge.targetNodeId })),
      ])
    ),
    incoming: Object.fromEntries(
      normalized.nodes.map(node => [
        node.id,
        (normalizedIncoming.get(node.id) ?? []).map(edge => ({ edgeId: edge.id, sourceNodeId: edge.sourceNodeId, handle: edge.sourceHandle || "default" })),
      ])
    ),
    topologicalOrder: topologicalOrder.length === normalized.nodes.length ? topologicalOrder : null,
    parallelGroups,
    loops: normalized.edges
      .filter((edge): edge is WorkflowEdge & { loop: WorkflowLoopPolicy } => Boolean(edge.loop))
      .map(edge => ({ edgeId: edge.id, sourceNodeId: edge.sourceNodeId, targetNodeId: edge.targetNodeId, maxIterations: edge.loop.maxIterations })),
  };
  return { ok: true, diagnostics: [], definition: normalized, plan, planHash: hashWorkflowExecutionPlan(plan) };
}

export function compileWorkflowDefinition(definition: unknown) {
...
export function validateWorkflowDefinition(definition: unknown, executable = false) {
  const result = analyzeWorkflowDefinition(definition, { executable });
  if (!result.ok) throw new WorkflowCompileError(result.diagnostics);
  return result.definition;
}

export function assertWorkflowExecutionPlan(plan: unknown, expectedHash: string) {
  if (!plan || typeof plan !== "object") throw new Error("已发布流程缺少不可变执行计划。");
