...
### 6.7 角色集体状态隔离与路由仲裁准则 (Role-Collective State Isolation & Routing Doctrine)

在状态流程（State Flow）中，必须严格贯彻“一类集体一状态，流向分歧归路由”的核心架构原则：

1. **角色集体状态专有性（Role-Collective Exclusivity）**：
   - 业务实体在生命周期中，任何时刻的责任所有权（Ownership & Custody）必须严格唯一；
   - 严禁模糊多角色混用的笼统状态（如“审核中”、“处理中”）；每一个业务参与人集体（经办人/发起人、直属主管、风控法务、财务核算、分管高管）都必须拥有其专属的状态节点：
     - 发起人/经办人集体：`[ 待经办人填报 ]`、`[ 待经办人补正材料 ]`
     - 直属主管集体：`[ 待直属主管初审 ]`
     - 专业合规集体：`[ 待风控合规审查 ]`
     - 财务中心集体：`[ 待财务会计复核 ]`
     - 高管决策集体：`[ 待分管领导终审 ]`
   - 状态节点表达的是**职责集体的等待与受理挂起状态**，必须使用等待语义（如“待XXX初审”），严禁使用正在进行时动作（如“XXX审核中”）混淆状态。

2. **充分发挥 Router（决策路由）的分流仲裁能力**：
   - **职责解耦**：操作节点（Operate）仅承载该集体的行为决策与审批意图（通过 / 退回 / 否决）；所有后继流向的选择、分支跳转与多维判定，必须完全交由 **Router 路由节点** 执行。
   - **Router 核心能力场景**：
     - **操作决策分流**：根据审批意图（`approved` / `rejected` / `returned`）分别导向下一级集体状态、补正状态或驳回终态；
     - **金额梯级路由（Amount Tiering）**：依据业务上下文中的金额、预算等数值变量，动态决定是直通基础审核还是升级至高层总监审批；
     - **业务类型与部门路由（Department/Domain Routing）**：依据报销类型、所属工作域智能分流至法务、技术或财务对应集体；
     - **退回回路保护**：主管退回后通过路由安全流转至经办人的“待补正”状态，经办人重新提交后再经路由准确回流至原主管，形成自闭环。

3. **发起人避嫌与四眼原则（Four-Eyes Principle / SOD）**：
   - 状态流程在流转至后续审批人集体状态时，必须在权限解析器与任务分配中自动剔除当前流程发起人/经办人，坚决杜绝“经办人自审自批”。

### 6.8 规范化状态流程示例（一集体一状态 + 决策/金额双层路由）

```text
[ 开始 (start) ]
   │
   ▼
[ 待经办人填报 (s_draft: 经办人集体) ]
   │
   ▼
[ 经办人提交材料 (op_submit: 经办人操作) ]
   │
   ▼
[ 待部门主管初审 (s_mgr_review: 主管集体) ]
   │
   ▼
[ 部门主管初审决策 (op_mgr_audit: 主管操作 - 通过/退回/否决) ]
   │
   ▼
[ 主管决策仲裁路由 (r_decision: Router 网关 1) ]
   ├── returned ──► [ 待经办人补正材料 (s_correction: 经办人集体) ] ──► (重新提交回流)
   ├── rejected ──► [ 已否决终止 (s_rejected: 终态) ] ──► [ end ]
   └── approved ──► [ 金额梯级决策路由 (r_amt_tier: Router 网关 2) ]
                      ├── 小额 (amount <= 5000) ───────────┐
                      │                                    │
                      └── 大额 (amount > 5000)             │
                           │                               │
                      ...
                      [ 财务总监会签操作 (op_cfo) ]        │
                           │                               │
                           ▼                               │
                      [ 待出纳付款记账 (s_finance_payout) ]◄┘
                           │
                           ▼
                      [ 财务执行付款 (op_payout) ]
                           │
                           ▼
                      [ 企业级归档办结 (s_archived: 终态) ] ──► [ end ]
```
