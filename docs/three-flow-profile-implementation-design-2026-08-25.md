...
- [ ] **3F-P0-020** 状态 Profile 强制至少一个状态、唯一初始状态和终态。
- [ ] **3F-P0-021** 操作节点增加 outcomes 和 Handle 校验。
- [ ] **3F-P0-022** 任务完成按任务组最终结果选择唯一分支。
- [ ] **3F-P0-023** 新操作节点拒绝进入拒绝分支，不再默认取消实例。
- [ ] **3F-P0-024** 旧节点保持 `legacy_cancel`，设计器提示迁移。
- [ ] **3F-P0-025** 新增状态迁移表和 currentState/stateVersion。
- [ ] **3F-P0-026** 状态迁移、任务完成、续跑 Job 使用同一事务。
- [ ] **3F-P0-027** 真实 MySQL 并发审批和重复推进测试。

### P1：人员、表单和时间

- [ ] **3F-P1-001** ParticipantResolver 注册表和可解释解析结果。
- [ ] **3F-P1-002** 部门、后代部门、负责人、N级主管、表单用户选择器。
- [ ] **3F-P1-003** 候选人快照与组织实时权限语义分离。
- [ ] **3F-P1-004** 设计器人员预览和空候选阻断。
- [ ] **3F-P1-005** 表单 Schema 版本和任务附着模型。
- [ ] **3F-P1-006** 任务 ownerVersion、转办和改派 CAS。
- [ ] **3F-P1-007** Timer、提醒、催办、升级和重启补触发。
- [ ] **3F-P1-008** 加签、减签、代理和责任主体审计。
...
- [ ] **3F-P1-025** LLM 成本、敏感字段、人工复核策略。
- [ ] **3F-P1-026** 状态节点从新控制流程目录移除，增加 Milestone。
