...
2026-08-22 重新扫描确认，裁剪副本保留 `assets/js/common/model.js`、`processDesign/processDesignCenter.js`、`dataFlowCanvas` 的管理脚本以及项目、仓库、系统配置、已启动流程脚本；但仍不含 `assets/js/designProcess/bundle.js`、`canvas.js` 和原始 CSS/图标资产。`model.js` 可直接证明流程列表、模块配置、数据源、资源、数据流调度查询/更新、任务详情、执行记录和节点执行记录等原始接口概念；`processDesignCenter.js` 可直接证明流程代号、名称、描述、类型、审核/发布状态、创建/发布取消时间、详情、发起/启动、取消发布、上传仓库、发布、设计、删除与审核重置的状态分支。当前实现已以内部账号、资源级权限和审计安全映射这些可见操作。

这些管理脚本**不包含**画布节点属性模板、默认值、帮助文本或节点级校验实现；因此不能提升开始、结束、状态、操作、路由、REST、SQL 等节点的字段证据等级，相关字段继续维持“兼容扩展”标注。
...
2026-08-19 已对当前共享项目目录、参考目录及 `/home/ubuntu` 可访问范围进行只读检索；未发现 `AiFlowGraph.zip`、原始 `assets/js/designProcess/bundle.js` 或原始 `canvas.js`。因此，本矩阵继续以裁剪副本中可读取的 HTML、保留脚本和历史控制器为唯一证据基线；完整归档到位前，节点内字段仍维持“兼容扩展”标注。

2026-08-22 再次确认项目共享目录为空，未找到完整 `AiFlowGraph.zip`、原始 CSS、图标、`designProcess/bundle.js` 或 `canvas.js`；本轮新确认的管理脚本仅补强页面与接口操作证据，不改变节点字段证据边界。
